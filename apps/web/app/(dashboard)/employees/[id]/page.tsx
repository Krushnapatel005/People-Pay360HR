...
export default function EmployeeProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const payslips: any[] = []; // getPayslips is on payrollApi, wait, it's not strictly needed for this view right now based on tabs

  if (loadingEmp) {
    return <div className="py-16 text-center text-slate-400">Loading profile...</div>;
  }

  if (!emp) {
    return <div className="py-16 text-center text-red-400">Employee not found</div>;
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-8">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Link
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-300 hover:text-white border border-slate-700 hover:border-slate-600 rounded-lg transition-colors"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-red-400 hover:text-red-300 border border-red-500/20 hover:border-red-500/40 bg-red-500/10 rounded-lg transition-colors"
              >
                Delete
              </button>
            </div>
