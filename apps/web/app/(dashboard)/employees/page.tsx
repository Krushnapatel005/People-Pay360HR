                  ...
                  >
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-500/30 to-brand-700/30 border border-brand-500/20 flex items-center justify-center text-xs font-bold text-brand-300 shrink-0">
                          {emp.initials}
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-white group-hover:text-brand-300 transition-colors truncate">{emp.fullName}</p>
                          <p className="text-[11px] text-slate-500 truncate">{emp.employeeRef}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 text-slate-400 hidden sm:table-cell">{emp.workEmail}</td>
      ...
      {view === 'kanban' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map((emp: Employee) => (
            ...
            >
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-500/30 to-brand-700/30 border border-brand-500/20 flex items-center justify-center text-lg font-bold text-brand-300 mb-3 group-hover:scale-105 transition-transform">
                {emp.initials}
              </div>
              <p className="text-xs font-semibold text-white truncate w-full">{emp.fullName}</p>
