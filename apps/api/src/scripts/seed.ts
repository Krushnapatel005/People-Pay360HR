import { PrismaClient, UserStatus, EmploymentStatus, ContractStatus, ScheduleType, AttendanceStatus, TimeOffUnit, RequestStatus, PayrunStatus, RuleCategory, CalculationMethod } from '@prisma/client';
...
async function main() {
  ...
  const getPerms = (codes: string[]) => permissions.filter((p) => codes.includes(p.code)).map((p) => p.id);
  ...
  const basicRule = await prisma.salaryRule.create({
    data: {
      ...
      category: RuleCategory.BASIC_SALARY, calculationMethod: CalculationMethod.FIXED,
    }
  });
  ...
  const pfRule = await prisma.salaryRule.create({
    data: {
      ...
      category: RuleCategory.DEDUCTION, calculationMethod: CalculationMethod.PERCENTAGE,
